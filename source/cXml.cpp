...
static bool getAttribute(std::shared_ptr<Xml::Tag> &tag, const std::string::const_iterator &begin, const std::string::const_iterator &end)
{
    for (auto iter = begin; iter != end; iter++)
    {
        ...
        if (*iter == '=')
            return ParseFail; // this attr without name
        // <[name] attr="value" >
        //         ^
        //         iter
        auto attrBegin = iter;
        ...
        {
            if (++iter == end)
                return ParseFail; //error: attr format missing "="
        }
        // <[name] attr = "value" >
        //             ^
        //             iter
        ...
        {
            if (++iter == end)
                return ParseFail; //error: attr format missing attr value
            if (*iter == '/')
            {
                return ParseFail;
            }
            ...
            else if (*iter == '\"')
            {
                ...
                {
                    if (++iter == end)
                        return ParseFail; //error: attr format missing close scope "
                }
                // <[name] attr = "value" >
                //                      ^
                //                      iter
                ...
                if (iter == end || *iter == '/' || *iter == '>')
                    return true;
                if (!xmlIsSpace(iter))
                    return ParseFail; //error: attr format missing space between two attr(<[name] attr0="value"attr1="value">)
                break;
            }
            else if (*iter == '\'')
            {
...
static bool parseExtern(const std::shared_ptr<Xml> &xml, std::shared_ptr<Xml::Tag> &tag, const std::string::const_iterator &begin, std::string::const_iterator &end)
{
    ...
    {
        if (*current == '?' || current == xml->toXmlString().end())
            return ParseFail; //error: xml format[tag without name]
        if (++current == stringEnd)
            return ParseFail;
    }
    //<? [name] attr="value"?>
    ...
    while (*current != '?')
    {
        ...
        { // get attribute
            ...
            {
                if (*current == '?' || *current == '>')
                    return ParseFail; //error: xml format[attribute without "="]
                if (++current == stringEnd)
                    return ParseFail;
            }
            //<?[name] attr="value"?>
            ...
            if (*current == '\"')
            {
                ...
                {
                    if (*current == '?' || *current == '>')
                        return ParseFail; //error: xml format[extern tag attribute not close with "]
                    if (++current == stringEnd)
                        return ParseFail;
                }
            }
...
static bool parseNode(const std::shared_ptr<Xml> &xml, std::shared_ptr<Xml::Tag> &tag, const std::string::const_iterator &begin, std::string::const_iterator &end)
{
    ...
    {
        // find "-->"
        // "-->" have no shared prefix and suffix, so no need to use KMP
    ...
    if (!res)
        return ParseFail;
    if (++current == stringEnd)
        return ParseFail; //error: xml format[tag without end tag]
    ...
    while (true)
    {
        ...
        if (*current == '<')
        {
            ...
            else
            {
                ...
                if (!res)
                    return ParseFail; //error: sub tag parse failed
...
bool Xml::update()
{
    ...
    if (iter == _fileCache.end())
        return false; //this xml object has all ready free
    ...
    if (!path.empty())
    {
        ...
        std::ifstream file(path);
        if (file.fail())
            return false; // read fail but don't clear [root]] and [extern tag]
        std::string res;
        std::string buf;
        while (std::getline(file, buf))
            res += buf + "\n";
...
{
    if (root == nullptr)
        return "Warning: this is not a xml string";
...
static std::string _toString(const unsigned int depth, const Xml::Tag *ptr)
{
    ...
    if (ptr->content.vaild())
        res += _indentation(depth) + "[content] " + ptr->content.toString() + "\n";
...
_XML_API std::shared_ptr<Xml::Tag> Xml::Tag::newTag(const std::string &name)
{
    ...
    {
        if (xmlIsSpace(iter))
            return std::shared_ptr<Xml::Tag>(nullptr);
    }
